    ...
    <div className="flex min-h-screen flex-col items-center bg-black px-6 py-16">
      ...
      <div className="mt-10 grid w-full max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2">
        {EXAMPLE_SERVERS.map((server) => (
          <div
            key={server.url}
            className="flex flex-col gap-3 rounded-lg border border-[#27272a] bg-black p-5"
          >
            ...
            <p className="text-xs text-white/60">by {server.publisher}</p>
            <p className="flex-1 text-sm text-white/70">{server.description}</p>
            <p className="truncate font-mono text-xs text-white/60">{server.url}</p>
            <Link
